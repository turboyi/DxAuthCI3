...
class DX_Auth
{
	...
	*/
	...
	function _set_session($data)
	{
		...
		$user = array(						
			...
			'DX_user_home'                      => $data->home,  // Add @ TURBOY 20160516			
	...
	*/
	...
	{
		return $this->ci->session->userdata('DX_user_home');
	}
	...
	{		
		...
		$new_user = array(			
			...
			'password'				=> crypt($this->_encode($password), '$1$' . $this->_gen_pass(8) . '$'), //FIX BUG: TURBOY 20150707
			'email'						=> $email,
		...
		if ($login)
		{
			...
			if ($query = $this->ci->users->get_login($login) AND $query->num_rows() == 1)
			{
				...
				if ( ! $row->newpass_key)
				{
					...
					$encode = crypt($this->_encode($data['password']), '$1$' . $this->_gen_pass(8) . '$'); //Fix BUG: force MD5 store into CHAR(34) by TurboY on 20150707
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if ($query = $this->ci->users->get_user_by_id($this->ci->session->userdata('DX_user_id')) AND $query->num_rows() > 0)
		{
			...
			if (crypt($pass, $row->password) === $row->password)
			{
				...
				// Old code:  $new_pass = crypt($this->_encode($new_pass));
				$new_pass = crypt($this->_encode($new_pass), '$1$' . $this->_gen_pass(8) . '$'); //Fix BUG: force MD5 store into CHAR(34) by TurboY on 20150707
